...
def convert_to_rasa_yaml(data, split):
    training_examples = [build_msg(e) for e in data]
    # in order to avoid dedup change sanitize_examples in TrainingData to `return examples` (instead of
    ...
    for lang in langs:
        ...
        for split in ["train", "valid"]:
            ...
            if write:
                ...
                rasa_nlu_data = convert_to_rasa_yaml(data, split)
                ...
                cleaned_slots_set = {replace_slash(s) for s in slots_set}
                if split == "train":
                    with open(f"{lang}/domain.yml", "w") as f:
                        rasa_domain_data = {
                            "version": "3.1",
                            "intents": list(set(intents)),
                            "entities": list(cleaned_slots_set)}
